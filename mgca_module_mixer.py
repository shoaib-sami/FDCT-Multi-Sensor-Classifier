...
class Restormer_Encoder(nn.Module):
    def __init__(self,
        ...
        self.relu = nn.ReLU()
        self.layer_norm1 = nn.LayerNorm([64, 224, 224])
        ...
        self.conv_detail=torch.nn.Conv2d(in_channels=64, out_channels=1,
            kernel_size=(1, 1))
        self.conv_projection=torch.nn.Conv2d(in_channels=3, out_channels=3,
    ...
    def forward(self, inp_img):
        ...
        base_feature1 = self.layer_norm1(base_feature1)

        base_feature = self.conv_base(base_feature1)

        detail_feature1 = self.detailFeature(out_enc_level1)
        detail_feature1 = self.layer_norm1(detail_feature1)
        
        detail_feature = self.conv_detail(detail_feature1)
        x=torch.cat((self.relu(base_feature), self.relu(detail_feature)), dim=1)
...
class MGCA(LightningModule):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(196*768*2, 10)
    def forward(self, img1,img2):
        '''Forward step of our method'''

        # Forward of query image encoder
        img1 = self.vit_cnn_feature1(img1)
        #print(img1.shape)
        #img1=self.conv1(img1)
        img_feat_q, patch_feat_q = self.img_encoder_q(img1)
        ...
        img2 = self.vit_cnn_feature2(img2)
        report_feat_q, word_feat_q = self.img_encoder_q(img2)
        word_attn_q = word_feat_q
        word_emb_q = self.img_encoder_q.local_embed(patch_feat_q)
        word_emb_q = F.normalize(patch_emb_q, dim=-1)
        report_emb_q = self.img_encoder_q.global_embed(img_feat_q)
        report_emb_q = F.normalize(img_emb_q, dim=-1)
        ...
        my_feature_cat_fc = self.fc(my_feature_cat)
        ...
        with torch.no_grad():
            ...
            img_code = self.get_assignments(
                img_code, self.hparams.sinkhorn_iterations).cuda()         # bz, 500
        ...
        return loss_ita, loss_local, loss_proto, acc1, acc5, my_feature_cat_fc
