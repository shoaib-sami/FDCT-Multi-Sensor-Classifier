...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=18, patch_size=4, in_chans=512, embed_dim=96, norm_layer=None):
        super().__init__()
        in_chans = 512
        img_size = (18,9)#to_2tuple(18)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #sattn_windows = self.grn(attn_windows)
        # merge windows
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.sparse_ratio = [0.9,0.7,0.5]#sparse_ratio
        self.pruning_locs = [1,2,3]#pruning_loc
        #print(" I am in predictor_list")
        predictor_list = [PredictorLG(dim) for i in range(len(self.pruning_locs))]
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=18, patch_size=4, in_chans=512, num_classes=10,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
...
dynamic_swin_v2 = AdaSwinTransformer(
    embed_dim=96,
    depths=[2,2],
    num_heads=[2,2],
    window_size=4,
    drop_rate=0.0,
    drop_path_rate=0,
    pruning_loc=[2,3,4], sparse_ratio=[0.9, 0.7, 0.5]
)
"""
a = torch.randn(10,512,18,9)
print(a.size())
d = model(a)
print(d[0].size())
"""